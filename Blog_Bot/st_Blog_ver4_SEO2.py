...
# SEO 점수 평가 기능 추가
if st.sidebar.button("SEO 점수 평가 요청"):
    if st.session_state.response:
        llm = ChatOpenAI(temperature=0.7, model_name='gpt-4o-mini')
        
        # SEO 평가 프롬프트 설정
        seo_prompt = f"""
        Please evaluate the SEO quality of the following blog post. Use Only Korean. Rate each of the criteria below on a scale from 0 to 10, and provide feedback and suggestions for improvement. Sum the scores for an overall SEO score (out of 50).
        ...
        1. **Keyword Density** (0-10): Does the post have a well-maintained keyword density for search optimization? Is the primary keyword appropriately distributed without keyword stuffing?
        
        2. **Meta Description Quality** (0-10): Is there a clear, engaging meta description that summarizes the blog post in a way that encourages clicks? Does it stay within 160 characters?
        
        3. **Header Tags** (0-10): Are H1, H2, and H3 tags used effectively to structure the content? Do they accurately represent the content hierarchy and improve readability?
        
        4. **Internal and External Links** (0-10): Are there relevant internal links to related content or external links to reputable sources? Do the links add value to the reader?
        
        5. **Readability and Engagement** (0-10): Is the content easy to read and engaging for the target audience? Does it use short sentences, active voice, and clear language?
        
        Please provide an overall SEO score along with specific suggestions for improving each of these elements.
        """
        
        # LLM에 프롬프트 전달하고 응답 받기
        seo_evaluation = llm.predict(seo_prompt)
        st.session_state.seo_evaluation = seo_evaluation  # 평가 결과를 세션에 저장

    else:
        st.sidebar.warning("블로그 포스트가 생성되지 않았습니다.")

# SEO 평가 결과 표시
if 'seo_evaluation' in st.session_state:
    st.markdown(f"""
        <div style="background-color:#FFF8DC;padding:15px;border-radius:10px;margin-top:15px;">
            <h4>🔍 SEO 점수 평가 결과:</h4>
            <p>{st.session_state.seo_evaluation}</p>
        </div>
    """, unsafe_allow_html=True)
