...
st.title("회의록 요약 및 변환 서비스")  # 서비스 제목
st.write("음성 파일 또는 텍스트 파일을 업로드하여 회의록 내용을 자동으로 구조화하고 요약합니다!")  # 간단한 설명
...
# ChatOpenAI 모델을 사용하여 회의록 구조화 요약 생성
def summarize_meeting(input_text): 
    ...
    # 회의록 구조화 프롬프트
    summary_prompt = f"""
다음 텍스트는 회의록 내용입니다. 이를 아래와 같은 구조로 요약해주세요:

1. **주요 의제**
    - 회의에서 논의된 핵심 주제를 요약합니다.
2. **참가자 발언 요약**
    - 주요 참가자들의 발언을 간략히 정리합니다.
3. **결론 및 다음 단계**
    - 논의된 결론과 후속 조치 계획을 요약합니다.
    ...
    if uploaded_file.size > MAX_FILE_SIZE_MB * 1024 * 1024:
        ...
        try:
            if uploaded_file.name.endswith(".txt"):  # 텍스트 파일 처리
                ...
                st.subheader("구조화된 요약")  # 요약된 텍스트 섹션 제목
